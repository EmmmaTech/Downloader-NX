...
TARGET		:=	Downloader
BUILD		:=	build.nx
SOURCES		:=	source lib/switch-zipper/source lib/cpr/cpr
DATA		:=	data
INCLUDES	:=	include $(BOREALIS_PATH)/library/include/borealis/extern/nlohmann lib/switch-zipper/include lib/cpr/include

# Output folders for autogenerated files in romfs
OUT_SHADERS	:=	shaders

OUTPUT_FOLDER := $(CURDIR)/out
...
CFLAGS	+=	$(INCLUDE) -D__SWITCH__ -D_DOWNLOADER_SWITCH \
...
LIBS	:= -lnx `curl-config --libs` -lz -lstdc++fs
...
export OUTPUT	:=	$(OUTPUT_FOLDER)/$(TARGET)
...
ifeq ($(strip $(NO_NACP)),)
	export NROFLAGS += --nacp=$(OUTPUT_FOLDER)/$(TARGET).nacp
endif
